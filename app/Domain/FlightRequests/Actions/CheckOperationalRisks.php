 ...
 * quote or confirmation request, a tight deadline with no confirmed
 * supplier, an expired quotation, an overdue invoice) is a direct
 * comparison against data Phases 6/8/10/12/17 already produce. The
 * genuinely judgment-based part of "should we worry about this supplier" —
 * reading their notes for red flags — lives in SupplierRecommender
 * instead, where it's actually useful: before a supplier is assigned, not
 * after.
 */
class CheckOperationalRisks
{
    /** How long a quote or booking-confirmation request can sit unanswered before it's flagged. */
    private const STALE_SUPPLIER_REQUEST_DAYS = 7;
    ...
    public function __invoke(FlightRequest $flightRequest): Collection
    {
        ...
        foreach ($flightRequest->services as $service) {
            ...
            if ($service->quote_requested_at !== null
                ...
                && $service->quote_requested_at->diffInDays(now()) >= self::STALE_SUPPLIER_REQUEST_DAYS
                && ! in_array($service->status, self::UNRESOLVED_STATUSES, strict: true)) {
                $findings->push(new OperationalRiskFinding(
                    field: "services.{$service->id}.quote_requested_at",
                    message: 'No quote received for '.$service->type->label().' in over '.self::STALE_SUPPLIER_REQUEST_DAYS.' days.',
        ...
        foreach ($flightRequest->supplierInquiries as $inquiry) {
            if ($inquiry->status === SupplierInquiryStatus::Chosen
                && $inquiry->confirmation_requested_at !== null
                && $inquiry->confirmed_at === null
                && $inquiry->confirmation_requested_at->diffInDays(now()) >= self::STALE_SUPPLIER_REQUEST_DAYS) {
                $findings->push(new OperationalRiskFinding(
                    field: "supplier_inquiries.{$inquiry->id}.confirmation_requested_at",
                    message: 'No booking confirmation from '.$inquiry->supplier->name.' for '.$inquiry->service->type->label().' in over '.self::STALE_SUPPLIER_REQUEST_DAYS.' days.',
                    why: 'The supplier may need a follow-up before this can be relied on as confirmed.',
                    affectedService: $inquiry->service->type->label(),
                ));
            }
        }
